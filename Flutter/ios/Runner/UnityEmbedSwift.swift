...
class UnityEmbeddedSwift: NSObject, UnityFrameworkListener, NativeCallsProtocol {
    ...
    private func initUnityWindow() {
        ...
        guard let loadedUfw = UnityFrameworkLoad() else { print("Error loading unity!"); return }
        ufw = loadedUfw
        ...
        ufw.appController()?.quitHandler = { print("Quit handler called!") }
    ...
    private func sendUnityMessageToGameObject() {
        ufw.sendMessageToGO(withName: "Sphere", functionName: "TestMessage", message: "works?")
    ...
    private func UnityFrameworkLoad() -> UnityFramework? {
        ...
        guard let bundle = Bundle.init(path: bundlePath) else { print("Error loading bundle!"); return nil }
        
        if !bundle.isLoaded == false {
            bundle.load()
        }

        guard let ufw = bundle.principalClass?.getInstance() else { print("Error getting instance!"); return nil }
